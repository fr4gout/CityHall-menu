...
export function IDCardPreview() {
  ...
  return (
    ...
    >
      <GlassCard className="relative w-full p-0 overflow-hidden glow-primary">
        <motion.div
          aria-hidden
        ...
        <div className="flex items-center justify-between border-b border-[var(--bd)] bg-[var(--bg-row)] px-5 py-3">
          <div className="flex items-center gap-2 text-[10px] uppercase tracking-[0.22em] text-[var(--primary)] text-glow">
            <Fingerprint className="size-3.5" />
            Government of San Andreas
          </div>
          <StatusBadge status={player.idCard.status} />
        </div>
        ...
        <div className="grid grid-cols-[112px_1fr] gap-4 p-5">
          ...
          <div className="space-y-2.5 text-sm">
            ...
            <Field k="Date of Birth" v={fmtDate(player.dob)} />
            <Field k="Phone" v={player.phone} mono />
            <Field k="Issued" v={fmtDate(player.idCard.issuedAt)} />
          </div>
        </div>
        ...
        <div className="flex items-end justify-between border-t border-[var(--bd)] px-5 py-3">
          ...
          <div className="text-[10px] tracking-wider text-[var(--tx-3)]">SA-DMV / CIV-ID</div>
        </div>
...
function Field({ k, v, mono }: { k: string; v: string; mono?: boolean }) {
  return (
    <div>
      ...
      <div className={mono ? "font-mono text-sm text-[var(--tx)]" : "text-sm font-medium text-[var(--tx)]"}>
        {v}
      </div>
    </div>
  );
